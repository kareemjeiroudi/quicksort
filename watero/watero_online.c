...
main() {
    ...
    #pragma omp parallel firstprivate(cputime) private(me)  
	{ 
		#pragma omp for private(i, j) reduction(+:energy) schedule(dynamic)
        ...
        printf("Elapsed CPU  time of thread %i: %f\n", me, (float) cputime / CLOCKS_PER_SEC);
    }
